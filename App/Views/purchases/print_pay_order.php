...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 15, 'PAY ORDER', 0, false, 'R', 0, 'B', 0, false, 'M', 'M');
        ...
        $this->Write(0, 'Farmer Name ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+24, $yVal);
        $this->Write(0, ': '.$this->record['farmer_name'], '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal, $yVal+4);
        $this->Write(0, 'Address ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+24, $yVal+4);
        $this->MultiCell(50, 1, ": ".$this->record['address'], 0, 'L', 0, 1, '', '', true);
    
        $this->SetXY($xVal, $yVal+16);
        $this->Write(0, 'NIC No ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+24, $yVal+16);
        $this->Write(0, ': '.$this->record['nic_no'], '', 0, 'L', true, 0, false, false, 0);
        ...
        $this->Write(0, ': '.PAY_ORDER_PREFIX."0".$this->record['id'], '', 0, 'L', true, 0, false, false, 0);
        
        $this->SetXY($xVal+150, $yVal+4);
        $this->Write(0, 'Date ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+180, $yVal+4);
        $this->Write(0, ': '.date("Y-m-d", strtotime($this->record['collection_date'])), '', 0, 'L', true, 0, false, false, 0);
...
$pdf->SetAuthor('PSC');
$pdf->SetTitle('Pay Order');
$pdf->SetSubject('Pay Order');
$pdf->SetKeywords('PayOrder');
...
// $pdf->MultiCell(55, 5, '[LEFT] '.$txt, 1, 'L', 1, 0, '', '', true);
    ...
    <tr style="font-size:12px; height: 20px;">
        <td border="0.5" style="text-align:left; width:50% height: 20px;">Item Name</td>
        <td border="0.5" style="text-align:left; width:15%">Qty</td>
        <td border="0.5" style="text-align:left; width:15%">Rate</td>
        <td border="0.5" style="text-align:left; width:20%">Total</td>
    </tr>
EOD;

    foreach ($record['items'] as $key=>$row):
        $name = $row['paddy_name'];
        $qty = $row['collected_amount'];
        $rate = $row['collected_rate'];
        $total = $total = $row['collected_amount'] * $row['collected_rate'];
...
EOD;
    ...
    $paid = $pay_order['paid_amount'];
    $table .= <<<EOD
    <tr style="font-size:12px;">
        <td border="0.5" colspan="3" style="text-align:left; width:80%">Sub Total</td>
        <td border="0.5" style="text-align:right; width:20%">$paid</td>
    </tr>
EOD;
