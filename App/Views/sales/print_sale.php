...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 15, 'SALE ORDER', 0, false, 'R', 0, 'B', 0, false, 'M', 'M');
        ...
        $this->Write(0, 'Customer Name ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+24, $yVal);
        $this->Write(0, ': '.$this->record['customer_name'], '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal, $yVal+4);
        $this->Write(0, 'Address ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+24, $yVal+4);
        $this->MultiCell(50, 1, ": ".$this->record['customer_name'], 0, 'L', 0, 1, '', '', true);
    
        $this->SetXY($xVal, $yVal+16);
        $this->Write(0, 'Email Address', '', 0, 'L', true, 0, false, false, 0);
        ...
        $this->Write(0, ': '.SALE_ORDER_PREFIX."0".$this->record['id'], '', 0, 'L', true, 0, false, false, 0);
        
        $this->SetXY($xVal+150, $yVal+4);
        $this->Write(0, 'Date ', '', 0, 'L', true, 0, false, false, 0);
        $this->SetXY($xVal+180, $yVal+4);
        $this->Write(0, ': '.date("Y-m-d", strtotime($this->record['issue_date'])), '', 0, 'L', true, 0, false, false, 0);
        

        $this->SetXY(5, $yVal+25);
        $this->Write(0, 'Collect your paddy from our '.$this->record['collection_center'].' collection center', '', 0, 'L', true, 0, false, false, 0);
...
EOD;
    ...
    foreach ($record['items'] as $key=>$row):
        ...
        $qty = $row['sold_amount'];
        $rate = $row['sold_rate'];
        $total = $total = $row['sold_amount'] * $row['sold_rate'];
